...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("version", 2, "unsupported_protocol_version"),
...
def test_protocol_status_does_not_claim_adapter_or_transport_readiness():
    status = worker_protocol_status()

    assert status["schema_ready"] is True
    assert status["adapter_connected"] is False
    assert status["transport"] == "not_implemented"
