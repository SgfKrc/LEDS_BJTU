...
> **主仓现行补充（2026-09-17）**：统一标准库 TUI 已在当前源码入口落地。`qlh` 无参/`qlh chat` 使用 `tui_admin.py` + `ChatScreen`，本机可由 `tui_backend.py` 在进程内启动后端；`DistributedScreen` 已接入 `PipelineLayout`。本文早期关于“7 屏、Textual 聊天独立进程、默认入口尚未完成”的段落属于历史网关/发布验收记录，现行启动和功能口径以 [TUI 使用指南](TUI使用指南.md) 为准；外部干净机、Linux 打包和真实分布式仍是发布/实验验收，不阻塞主仓 TUI 使用。
...
`tui_admin.py`（70KB，纯标准库）是 QLH 的终端管理面，经 HTTP 操作集群；历史网关基线覆盖 7 个管理屏，当前主仓统一 TUI 另加 `ChatScreen` 共 8 屏。主计划承诺"TUI 零改动"——TS 网关替换 FastAPI 后 TUI 照常运行。2026-08-02 已完成 TUI 依赖面审计（38 处调用点 / 25 个唯一端点），结论：
