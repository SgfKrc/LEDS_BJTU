...
| **Qwen3.5-9B（双格式）** | GGUF Q4_K_M 5.68 GB + 权重 19.3 GB | 蒸馏主选学生（9B）、实验 | ModelScope `unsloth/Qwen3.5-9B-GGUF`（Q4_K_M）+ `Qwen/Qwen3.5-9B`（权重）；本地已收集 ✓ | 蒸馏/实验 |
| **Qwen3.5-2B（GGUF，本地转换）** | 1.31 GB | 蒸馏玩票对照组 | 本地转换（MODEL-TOOLS gguf-convert，官方无 GGUF）；本地已转换 ✓ | 玩票 |
| **Gemma4 12B 权重（Safetensors）** | 23.9 GB | 原生绑定权重源（对照 GGUF 路径） | ModelScope `google/gemma-4-12b-it`；本地已收集 ✓ | 可选 |
...
> 💡 **懒人入口**：具备所需本地资产的发布者可先运行 `python scripts/build_offline_bundle.py --pc --preflight --verify` 检查容量，再按需构建 PC/Android 整合包。脚本不下载或修改资产；若某项缺失，先执行 `--missing` 获取清单。克隆者可优先从分发服务器下载已验证的整合包，详细结构和发布复验见[离线资产一键整合包设计](离线资产一键整合包设计.md)。
