...
> 状态：**已实施（PATCH-01～06 全部完成，2026-09-14；PATCH-03 的 RAG 对接部分待知识库服务化后接）**；PATCH-07 **已实施**（父文档映射 + rewrite/rerank 参数对照，真实对照 MRR 0.33→0.44）；PATCH-08（启动动画）已登记；本计划把 docagent（只读扫描引擎）与主项目 `docs/` 文档库接成可交互的"图书馆管理员"工作台。
>
> 创建日期：2026-09-14
