...
| 多模态（图像生成） | SD 1.5 全套（文生图/图生图/IP-Adapter/inpaint/指令编辑） | ❌ 无 | 🔴 大 |
...
- 安卓：无。
- 差距点：SD 在安卓端可行性本身存疑（diffusers PyTorch 栈 + 显存），**建议不排期**（列为"明确不做"以省资源）。
...
| **P3** | D 本地判题（Qwen3-4B GGUF 安卓版） | 可选实验线 | 无 |
| **P3** | F 应用内更新/日志 | 运维体验 | 无 |
| **不做** | A2 图像生成（SD） | 安卓端可行性存疑，明确不做省资源 | — |
