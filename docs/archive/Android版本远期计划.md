...
近期目标不是让 Android 假装成 PC PyTorch 层节点，而是先让它成为可靠、可取消、可重派、可统计的完整 Worker。任务链拆分随后提供真正可维护的异构分布式推理；层间拆分则保留为有严格准入门槛的研究方向。PC、Android、GGUF stage 和张量并行执行池的统一选择规则见[混合分布式推理体系规划](archive/混合分布式推理体系规划.md)。
