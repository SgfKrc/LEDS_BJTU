...
| 网络搜索 | `web_search` 在本机 capability catalog 中不可用；Reasonix 报告没有搜索后端。**归属确认**：按内置文档 `docs/WEB_SEARCH.md`，搜索是 **provider 侧能力**（另发一次模型请求、使用后端原生搜索，查询交给 provider 并按搜索请求计费） | **不在 bridge 自建任何搜索后端**；接通 provider 时只做透传，未接通时稳定返回 unavailable——细节见 [工具面现状](../../../docs/archive/misc/reasonix-codex-bridge工具面现状-2026-09-13.md) |
