...
> **终态说明（2026-09-14）**：本计划是 QLH 主项目的历史记录。主项目已移除 SD 资产打包、下载和发布接线；生图资产与依赖只由 Koakumix 管理。本文中的路径、脚本和发布状态不应作为当前 QLH 操作说明。
...
>
> 总计划入口：[总体下一步计划](../distributed/总体下一步计划.md) L4-SD1.5；专项背景见 [SD 1.5引擎与分布式图像生成实施计划](SD%201.5引擎与分布式图像生成实施计划.md)
