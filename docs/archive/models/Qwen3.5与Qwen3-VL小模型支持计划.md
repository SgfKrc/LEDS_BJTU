...
>
> `EX-QW3V2-01`（2026-09-12）已完成 v2 全口径补测：Qwen3-4B Q4_K_M 使用 `ps-qwen3-nothink`、`loose_contains`、`max_new_tokens=512` 和固定采样 `0.7/0.8/20/0` 串行三轮，correctness **1/4 ×3**、format **7/11 ×3**，波动为 0。证据 `build/experiments/ex-qw3v2-01-20260912/`；该结果更新 QW3-E4 的质量基线，但不自动恢复生产质量门。
>
> 更新日期：2026-09-12
>
> 适用范围：Qwen3 文本系列、Qwen3-VL 系列与 Qwen3.5 原生多模态系列的工件获取、引擎准入和 EX-N3 质量标定。Gemma 4 的新格式工件另立候选票，不在本文引用或预设未建立的 Gemma 子票。
>
> 总计划入口：[总体下一步计划](../distributed/总体下一步计划.md)；下载与用户代理规则：[一键模型部署与自治集群远期计划](../../qlh-release/docs/一键模型部署与自治集群远期计划.md) §7.1。
...
> **2026-08-14 `PT-PIPE-QW3.17` 增量复核通过**：真实隔离 `Qwen3NetworkSidecarExecutor` 已在 helper 进程内完成 2/3 节点 CPU torch artifact 链，覆盖 prefill/decode、KV generation、path-free output reference、重复 consume 幂等和 revoke 清理；真实模型测试新增 xdist 串行锁，避免并发 RAM 竞争造成假失败。真实链专项 `39 passed / 1 skipped`、QW3 全线 `159 passed / 3 skipped`。Qwen3.5/Qwen3-VL 仍未接入该执行面；下一票 `PT-PIPE-QW3.18` 专注真实 PATCH 断线续传与 ledger progress/revoke 终态。

> **2026-08-14 `PT-PIPE-QW3.18` 开发门完成**：普通文件 handoff 已把 receiver 确认 offset 写入目标 ledger；断线重试从 `.part` 确认位置继续，已提交状态的重复 progress/status/commit 不回退且幂等。真实 helper 场景模拟 PATCH 已落盘但 ACK 丢失，150,000 字节工件续传、SHA/字节与 ledger 终态均通过；QW3 全线 `161 passed / 3 skipped`，sidecar/network/真实 helper 合并专项 `40 passed / 2 skipped`。下一票进入 `PT-PIPE-MM1`，先设计 Qwen3-VL `mmproj`/processor 与 Qwen3.5 视觉组件的 path-free artifact contract；真实双机/CUDA 与生产准入继续关闭。

> **2026-08-14 `PT-PIPE-MM1.1/.2` 开发门完成**：Qwen3-VL/Qwen3.5 已有 path-free 多模态组件 manifest、视觉到文本 hidden handoff、容量预算和 QW3 committed reference binding；本地 config 核对为 Qwen3-VL-4B 文本 36 层/hidden 2560、Qwen3.5-2B 文本 24 层/hidden 2048，未加载权重。MM1 定向 `6 passed`，QW3+MM1 联合 `169 passed / 1 skipped`。下一票 `PT-PIPE-MM1.3` 用合成视觉组件接入隔离 sidecar/consume 与重试、撤销、TTL/epoch 清理矩阵；真实图像/视频、CUDA/双机 parity、生产准入继续关闭。

> **2026-08-14 `PT-PIPE-MM1.3` 开发门完成**：目标侧 `Qwen3MultimodalSyntheticExecutor` 已接入现有 QW3 consume callback，在本地生成确定性小工件并校验 MM1 visual→text binding；执行失败后 ledger 为 `failed` 且输出清理，peer epoch/TTL 终态分别为 `invalidated`/`expired`。MM1 runtime `3 passed`，QW3+MM1 联合 `172 passed / 1 skipped`。下一票 `PT-PIPE-MM1.4` 接入真实隔离 Transformers sidecar metadata；当前仍不宣称 Qwen3-VL/Qwen3.5 真实图像/视频质量或 CUDA/双机 parity。

> **2026-08-15 `PT-PIPE-MM1.4` 开发门完成**：现有真实隔离 Qwen3 sidecar 已通过 `Qwen3MultimodalSidecarAdapter` 输出 MM1 binding SHA，覆盖 CPU prefill/decode、path-free 控制结果和 cleanup 转发；真实 MM1 sidecar smoke `1 passed`，MM1 runtime `4 passed`，联合 `174 passed / 1 skipped`。本票仍以 Qwen3-4B hidden artifact 验证 metadata 接线，未执行 Qwen3-VL/Qwen3.5 视觉塔。下一票 `PT-PIPE-MM1.5` 固定 processor/视觉 worker 离线 preflight schema。
...
| QW3-E4 | Completed（v1；v2 补测见 `EX-QW3V2-01`） | Qwen3-4B 三轮客观子集标定与人工复核 | v2 已完成三轮稳定证据；质量门仍需独立人工复核与显式阈值变更 |
