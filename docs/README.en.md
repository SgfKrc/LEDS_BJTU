...
| 🔌 **Reasonix ↔ Codex bridge sub-project** | Standalone package (own repo [reasonix-codex-bridge](https://github.com/SgfKrc/reasonix-codex-bridge), brought in as a submodule): exposes the read-only Reasonix subagent to Codex as stdio MCP tools (`reasonix_run` / `reasonix_status`); CLI path and model ref resolve per machine with zero hard-coding (`node src/configure.mjs list/use/codex/verify`); write access stays off by default, controlled writes tracked as `TOOL-RXB-W1/W2/W3` → [roadmap](reasonix-codex-bridge完善方向-2026-09-12.md) |
