...
| L-to-L upstream channel | The pip-bound `llama_get_embeddings_ith` returns `output_norm(H)` (measured cos 0.999998), so it **cannot** serve as a layer-relay upstream; the unified driver `scripts/relay_experiment.py` fails loud for `l2l_llama` by default, and the L-to-L upstream needs a self-built llama.cpp keep-head channel |
