...
| 📝 **Document-maintenance Agent sub-project** | Standalone `tools/docagent/` package: rule-as-data (`RULES.md` + `rules.yaml` driven scanner), rule-change mechanical scanning (delta matrix new/gone/changed + `--max-new/--max-gone` gates), evolution gates (agent rule edits: proposed→preflight→gates→released) with equivalence regression → [plan](文档维护Agent工具子项目化与通用化专项计划.md) |
