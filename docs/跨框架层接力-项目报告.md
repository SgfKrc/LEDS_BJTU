...
**两台机器上的实现边界（重要）**：
- **Android** = 另一条路线（llama.cpp RPC **借算力**），**不是**层接力节点：主仓**硬性排除** Android 参与层拆分（`src/scheduler.py:3754`「阻止 Android 节点被分配层（Android 无 PyTorch 推理能力）」、`:2620`「仅 PC 节点参与层拆分，Android 节点跳过」）。其唯一计算形态是**整请求级**的 `full_inference` stage（`android_full_worker`，加载整模）。
...
**未完成 / 待办**
1. **生产门仍 fail-closed 关闭**；准入需要：prompt 分布统计（单 prompt 结论不可泛化）、长序列、弱网断线、跨节点时钟与协议版本一致性；
2. 切点仍**固定 4 层**（而实测最优在 N=20）—— 权重占比也不合理：`L0-3` 仅 **10.1%**、`L4-23` 占 **50.3%**；
3. **tied 模型的末节点**（§5.3 第 14 条）需修；
