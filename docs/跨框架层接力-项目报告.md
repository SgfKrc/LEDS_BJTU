...
> **当前主仓双引擎样本**：qwen2.5-0.5B、12+12 层、gen=32，上游 `model_module.forward_layers`、下游 `llama_engine.forward_layers_from_hidden`，D→L **47.501 ms/步，token 一致**（`local_docs/CORE-RELAY-XFRAME-02-swap-ld-2026-09-19.json`）。**2026-09-21 复跑**：在同一公共接口上补齐完整矩阵（两模型 × 切点/负载/batch/混合精度）**27/27 逐 token 一致**，qwen3.5 hybrid 的 K=8/12/16/20 四个切点全部 **32/32**；旧的 21.3 s / 333 ms/步是历史优化链，不得替代此口径。
