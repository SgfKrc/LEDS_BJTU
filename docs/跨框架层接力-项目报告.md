...
> **状态：持续更新**（当前基线复核：2026-09-21）
> 配套文件：[`跨框架层接力-实验数据汇总.md`](archive/relay/跨框架层接力-实验数据汇总.md)（全部数字与测试条件；本报告只引其中结论，不重复列数字）
...
> ⚠️ **有效性**：本文保留 9 月 18–19 日的实验过程和历史口径；当前正确性/容量索引以 [`跨框架接力当前有效基线与后续优化计划`](跨框架接力-当前有效基线与后续优化计划-2026-09-21.md) 为准。`admit_relay_xframe` 现按正确性证据准入，速度只影响默认路由倾向；长时运行、远端资产分发和多段故障验收仍未完成。

> **当前主仓双引擎样本**：qwen2.5-0.5B、12+12 层、gen=32，上游 `model_module.forward_layers`、下游 `llama_engine.forward_layers_from_hidden`，D→L **47.501 ms/步，token 一致**（`local_docs/evidence/relay-xframe/CORE-RELAY-XFRAME-02-swap-ld-2026-09-19.json`）。**2026-09-21 复跑**：在同一公共接口上补齐完整矩阵（两模型 × 切点/负载/batch/混合精度）**27/27 逐 token 一致**，qwen3.5 hybrid 的 K=8/12/16/20 四个切点全部 **32/32**；旧的 21.3 s / 333 ms/步是历史优化链，不得替代此口径。
...
| ③ | **跨框架层接力**（当前主线）：按**层**切分，上游用 PyTorch 跑 `embed + L0..N-1`，把 hidden state 交给下游的 llama.cpp 跑 `LN..last`，**各节点只持有自己那段层的权重** | 研究推进中；正确性证据可准入，速度只影响默认路由，长时/远端资产/多段验收仍待完成 | 见 §3、§6 和当前基线索引 |
...
| 生产门 | `admit_relay_xframe` 按正确性证据判定；未验证或触发硬门的请求仍 fail-closed，速度不再单独否决 |
...
14. **tied embeddings 的末节点缺张量**（B13 发现，**已由 B16 修复**）—— Qwen3.5 的 safetensors 没有独立 `lm_head.weight`；当前实现探测 tied embeddings，用 `embed_tokens` 充作末节点 `lm_head` 并保持共享，旧异常不得再作为当前缺口引用。
...
19. ⚠️ **被后续实测撤销的结论清单**（`local_docs/evidence/relay-xframe/CORE-RELAY-XFRAME-02-TODO-v2-2026-09-18.json` 集中列出）：B1 的「长序列 compile 劣化」、B6 的「static cache 最优」、cache-mechanism 之前的「compile 下 hybrid 恒定慢 1.5×」（实为**快** 1.57×）、「IPC 是瓶颈」、「不接力最快」、「退出期崩溃」、「上游逐层慢 3×」、以及 README 里「慢约 20×」的口径。
...
- **B13 —— Qwen3.5 的 hybrid 递归状态在层流水线里的传递**（**已完成**）：**结论是「段间不需要传任何 cache」** —— 契约本就如此（`past_key_values` 索引对应**本节点本地层**；`_kv_cache` 按 `task_id` 存「本节点层范围的 KV」）。hybrid 的 `linear_attention` 层不写 KV、用 conv/recurrent state，**只依赖本层的输入序列** ⇒ 只要 **prefill 阶段上游把完整序列的 hidden 交给下游**，下游就能从 token 0 正确累积 ⇒ **hybrid 不引入额外的跨节点状态传输需求**。
  ...
  **副产品（已修复）**：B16 已让 tied 模型以共享 `embed_tokens` 充作末节点 `lm_head`；Qwen3.5 末节点不再是当前阻塞。相关回归见 `tests/test_tied_word_embeddings.py`。
...
**未完成 / 待办**
1. **生产门已按正确性证据准入**（2026-09-21）：原先把性能窄化为速度的拒绝理由已撤销；容量收益属于性能，且已由独立报告量化。**默认路由仍优先可直接整模的 L/RPC**，这不是对已验证 Relay 正确性的否定；
   ...
   * **跨节点时钟与协议版本一致性** ✅ 已补（本机范围）：`out/xframe-consistency.json` 固化下游 llama.cpp 构建指纹（commit `6f04274cc` / branch `pr-27073` / **8 个本地补丁及其 SHA-256**）、Relay 线协议常量（`QLHR` / `wire_version=1` / `float32_le` / header `!4sBBHIIQ`）、上游指纹与时钟基准。**结论：两侧必须同构建**——实验 fork 带本地补丁，构建不一致时逐 token 一致结论不成立 ⇒ 跨节点部署必须核验该指纹。
    ⇒ 四项齐备后合同返回 `admitted`；速度不占优只进入路由倾向，不能否定容量合并能力（合同实现见 `src/relay_contract.py`）。
...
4. Android 路线（借算力）设备门已通过，但 APK 内部 `layer_forward`、热/电、弱网和长时证据仍待补；它与层接力**互不替代**。
...
- 票表：`docs/验收清单与资源限制登记.md`（D29）、`local_docs/evidence/relay-xframe/CORE-RELAY-XFRAME-02-TODO-v2-2026-09-18.json`
