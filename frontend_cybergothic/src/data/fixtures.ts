...
export const modelPresetsFixture: ModelPresetsResponse = {
  presets: [
    {
      ...
      resource_gate: { min_ram_gb: 4, min_vram_gb: 0, min_disk_gb: 2, allow_cpu: true },
      description: 'CPU 友好的低显存模型。',
      installable: true,
      blocked_reasons: {},
    },
    {
      ...
      resource_gate: { min_ram_gb: 10, min_vram_gb: 8, min_disk_gb: 10, allow_cpu: false },
      description: '需要 CUDA 与较大显存。',
      installable: false,
...
const fixtureDownloadJob: ModelDownloadJob = {
  job_id: 'fixture-download-01',
  status: 'downloading',
  progress: 0.42,
  source: 'Qwen/Qwen-1.8B-Chat',
  target: 'models/qwen-1_8b',
  model_id: 'qwen-1_8b',
  preset_id: 'qwen-1_8b-gguf-q4',
  engine: 'llama_cpp',
  quant: 'Q4_K_M',
  downloaded_bytes: 420 * 1024 * 1024,
  total_bytes: 1000 * 1024 * 1024,
};

export const modelDownloadsFixture: ModelDownloadsResponse = {
  jobs: [fixtureDownloadJob],
};

export const modelSearchFixture: ModelSearchResponse = {
  query: 'qwen',
  source: 'all',
  provider: 'hf',
  total: 2,
  fallback_used: false,
  attempts: [{ provider: 'hf', transport: 'direct', status: 'ok' }],
  results: [
    {
      id: 'Qwen/Qwen3-4B-GGUF',
      source: 'hf',
      display_name: 'Qwen3 4B GGUF',
      description: 'Community GGUF package for CPU-friendly inference.',
    ...
    },
    {
      ...
      description: 'Safetensors package for Transformers.',
      tasks: ['text-generation'],
      tags: ['safetensors', 'qwen3'],
      downloads: 9200,
      likes: 215,
      size_bytes: 8_600_000_000,
      last_modified: '2026-08-17T08:20:00Z',
      license: 'apache-2.0',
      private: false,
      gated: false,
      url: 'https://modelscope.cn/models/Qwen/Qwen3-4B',
    },
  ],
};
