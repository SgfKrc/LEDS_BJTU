...
| 生产可用性 | ❌ 当前最优仍**慢约 20×**（差距已从 180× 收窄）；**生产准入保持 fail-closed 关闭** |
...
- **不接力反而最快**（186.8 ms/步，比默认 N=4 快 12.9%）⇒ 本机同机、单序列条件下，接力开销约 **+15%**（相对纯 llama.cpp CPU 口径）。这比"对比 llama.cpp 原生 GPU 慢约 20×"温和得多——**那 20× 主要是 CPU/GPU 之差，不是接力机制的成本**；
