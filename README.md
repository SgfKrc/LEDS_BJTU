...
**累计实测（gen=64，逐 token 全部一致）**：

| 配置 | 墙钟 | 相对首版 |
| --- | ---: | ---: |
| 跨进程 + 全段重算 | 182 s | 1.0× |
| 同进程 + 全段重算 | 167 s | 1.09× |
| 同进程 + 双侧 KV 复用 | 53 s | 3.4× |
| **同进程 + KV + 上游手动 4 层** | **21.3 s（约 333 ms/步）** | **8.5×** |
| 跨进程 + KV + 4L + 下游 GPU offload | 81 s | 2.2×（受 IPC 拖累） |
| *对照：llama.cpp 原生整模 GPU* | *约 0.9 s* | *约 200×* |

**证据与边界（如实说明，均来自已落盘的实测）**：

| 项 | 结果 |
| --- | --- |
| 正确性 | ✅ 跨进程、同进程、跨机（SSH 隧道）与 f32 对照均逐 token 一致；**上游手动 4 层 bit-exact**（最大绝对差 = 0） |
| 耗时剖析 | **99.3% 是两侧的实际计算**（上游 66.5% + 下游 32.9%）；通信、同步与 batch 管理合计仅 **0.34%** |
| 最大单项收益 | ✅ 上游此前在**白跑 20 层**（跑满 24 层却只用前 4 层输出）；改为手动 4 层前向后上游 **10.7×**（407.6 → 38.1 ms/步） |
| 层流水线 | ✅ 上游只加载 `embed_tokens + L0-3`：**1.47 GB（f16）**，对整模 4.55 GB **3.1× 更小**，端到端 64/64 一致 |
| 已证伪 | ❌ 消除进程边界（仅 8%，且是"两侧都慢"时的假象）、复用 `llama_batch`（0.09%）、朴素截断上游层（数值崩坏）、`--override-tensor` 提速（实为容量旋钮） |
| 生产可用性 | ❌ 当前最优仍**慢约 25×**（`333 ms/步 ÷ 13.5 ms/步 = 24.7`；该参照来自 09-16 报告的 `llama.cpp 原生整模 GPU`，**与本 README §P2 表的 `26.6 ms/token` 不是同一参照** —— 后者口径下为 12.5×，见实验数据汇总 §H）。距最初 **61×**（`828 ms/步 ÷ 13.5`）已大幅收窄；**生产准入保持 fail-closed 关闭** |
| 算子环境 | ✅ 缺失算子可绕开：WSL2（Ubuntu-22.04）GPU 直通与 `triton`/fla 已实测可用 |
| 当前定位 | **架构兼容轨道**；默认关闭，不替代 RPC，不进入 Edge 默认路由；优化项登记在 [验收清单 D29](docs/验收清单与资源限制登记.md) |

**结论修正**：早期"IPC 是主要成本"的判断已被推翻——那是两侧都慢时被掩盖的假象。**杠杆在两侧的计算（切点、kernel、批量），不在传输层面**。详见 [同进程双后端接力实现与性能](docs/同进程双后端接力实现与性能-2026-09-16.md) §12–§14。
...
| 跨框架逐层接力（D→L） | 正确性多路验证（跨进程/同进程/跨机 SSH/f32 均逐 token 一致；上游手动 4 层 bit-exact）；累计 **8.5×**（21.3 s / 64 步），当前最优仍慢约 **25×**（`333 ms/步 ÷ 13.5 ms/步`；**CPU 上游**口径）。⚠️ 参照物不止一个，**不可互相印证**：本 README §P2 表的 `26.6 ms/token` 口径下为 **12.5×**，GPU 上游 N=20（`129.1`）对该参照为 **4.85×** —— 详见实验数据汇总 §H，**生产准入 fail-closed 关闭**；优化票见 D29 |
...
- [同进程双后端接力实现与性能](docs/同进程双后端接力实现与性能-2026-09-16.md)
